...
public class directory {
	...
	//Creates two Lists, a list of files and a children to distinguish between 
	...
	//This function is used to set the first specified directory equal to the second specified
	//directory. It doesnt affect the second directory in any way ,only the first one needed
	//because dir is a custom class which the "=" operator dies not support needed in main function
	...
	//Thsi function is for ls and prints out all of the files in the named directory
	...
	public void directory_size(String name){
		...
		if(dir_name.equals(name)){
			if(myList.isEmpty()==false && children.isEmpty()){
				...
				System.out.println("There are no files or sub directories to print: \n");
			}
			...
			for(int i=0;i<children.size();i++){
				if(name.equals(children.get(i).dir_name)){
					if(children.get(i).myList.isEmpty() && children.get(i).children.isEmpty()==false){
						...
						System.out.println("there are no files or sub directories in the specified directory");
					}
				}
				else{
					count++;
